...
# 使用encode()将str转换为bytes，bytes就是一串hex数据，主要用于网络传输，同一个str根据编码方式不同可以转换为不同的bytes
# 使用decode()将bytes转换为str，单纯的bytes是没有意义的一堆二进制数据，只能知道了编码方式才能解析
# web传输一般用UTF-8编码解码：bytes→str（UTF-8解码）
# 串口传输一般用ASCII编码解码：bytes→str（ASCII解码）→int
...
print("将'中文'转换为unicode_escape编码：", b.encode('unicode_escape'))
