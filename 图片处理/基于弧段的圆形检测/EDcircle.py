...
def main():
    ...
    # ---------- 修改部分：兼容 OpenCV 4 和 5 的 ellipses 形状 ----------
    if ellipses is not None:  # 检查是否找到圆和椭圆
        # 统一形状为 (N, 6)
        if ellipses.ndim == 3 and ellipses.shape[1] == 1:
            # OpenCV 4 常见格式: (N, 1, 6)
            ellipses = ellipses[:, 0, :]          # -> (N, 6)
        elif ellipses.ndim == 2 and ellipses.shape[1] == 6:
            # OpenCV 5 可能直接返回 (N, 6)
            pass
        else:
            # 尝试强制重塑为 (N, 6)（若形状未知，可能出错，但大多数情况适用）
            ellipses = ellipses.reshape(-1, 6)
        ...
        # 现在 ellipses 是 (N, 6)，直接迭代每一行
        for ell in ellipses:
            center = (int(ell[0]), int(ell[1]))
            # 注意：原代码中轴长度计算方式为 ell[2]+ell[3] 和 ell[2]+ell[4]
            # 这可能是为了适应特定返回值，保留原逻辑
            major_axis = int(ell[2]) + int(ell[3])
            minor_axis = int(ell[2]) + int(ell[4])
